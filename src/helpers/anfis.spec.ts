import Anfis from './anfis'

describe('test', () => {
  it('test', () => {
    // 0.04681238768287473

    const result = new Anfis(
      4,
      5,
      [
        [1.699, 1.76, 0.5004], // 0,0
        [1.063, 2.425, 3.216], // 1,0,
        [0.7791, 2.071, 4.075], // 2,0
        [0.8593, 2.277, 6.205], // 3,0
        [1.622, 2.025, 8.922], // 4,0
        [2.28, 0.2141, 1.972], // 0,1
        [1.884, 2.441, 5.44], // 1,1
        [0.6311, 2.858, 7.893], // 2,1
        [1.325, 2.799, 9.67], // 3,1
        [1.963, 1.743, 13.34], // 4,1
        [2.368, 0.8655, 21], // 0,2
        [1.72, 2.43524, 24.81], // 1,2
        [0.671, 2.13, 27.7], // 2,2
        [1.504, 2.74, 31.4], // 3,2
        [2.247, 1.702, 35.24], // 4,2
        [5.364, 0.4197, 1.182], // 0,3
        [5.237, 1.964, 11.44], // 1,3
        [5.375, 2.877, 22.15], // 2,3
        [5.498, 1.9, 32.07], // 3,3
        [5.474, 2.444, 41.34], // 4,3
      ],
      [
        [1, 1, 1, 1, 1], // [p1, q1, r1, s1, t1]
        [1, 1, 1, 1, 1], // [p2, q2, r2, s2, t2]
        [1, 1, 1, 1, 1], // [p3, q3, r3, s3, t3]
        [1, 1, 1, 1, 1], // [p4, q4, r4, s4, t4]
        [1, 1, 1, 1, 1], // [p5, q5, r5, s5, t5]

        // [1, 2, 3, 4, 5], // [p1, q1, r1, s1, t1]
        // [1, 2, 3, 4, 10], // [p2, q2, r2, s2, t2]
        // [1, 2, 3, 4, 15], // [p3, q3, r3, s3, t3]
        // [1, 2, 3, 4, 20], // [p4, q4, r4, s4, t4]
        // [1, 2, 3, 4, 25], // [p5, q5, r5, s5, t5]
      ]
    )

    expect(result.feedForward([4.5, 7.8, 28.5, 25])).toBe(1)
  })
})
