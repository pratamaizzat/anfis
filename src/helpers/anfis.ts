...
export default class Anfis {
  ...
  constructor(nodeInput: number, nodeMF: number, premis: number[][]) {
    ...
    // generate mf
    this.mf = new MembershipFunction(this.nodeMF, this.nodeInput, this.initialPremis)
  ...
  feedForward(inputTest: number[]): number {
    ...
    const out1 = MembershipFunction.execute(input, this.mf, this.mf.getPremis)

    // console.table(input.matrix)
    // console.table(this.mf.getData.matrix)
    // console.table(this.mf.getPremis.matrix)
    console.table(out1.matrix)
